...
def animate_frames(frames):
  env_player = EnvPlayer(frames_only=True)
  for frame in frames:
    env_player.play_frame(frame)
    time.sleep(0.01)
  env_player.play_frame(frame, done=True)
  env_player.close()
  return
...
def collect_trajectories(envs, policy, tmax=200, nrand=5, verbose=1):
  t0 = time.time()
  # number of parallel instances
  n=len(envs.ps)
  if verbose == 1:
    print("Collecting {} steps from {} workers...".format(tmax, n), flush=True)
  elif verbose == 2:
  ...
  t1 = time.time()
  if verbose == 1:
    print("Collected {} steps from {} workers in {:.1f}s".format(
        len(prob_list), n, t1-t0), flush=True)
  elif verbose == 2:
    print(".", end="", flush=True)
